"use client";

import React, { useEffect, useState } from "react";
import {
  Table,
  TableBody,
  TableCell,
  TableHeader,
  TableRow,
} from "../../ui/table";
import * as XLSX from "xlsx";

// Função para buscar os dados com datas dinâmicas
async function fetchPowerData() {
  const res = await fetch(
    `https://gonansen.com.br/endpoints/central-monitoring/power-data/`,
    {
      headers: {
        accept: "application/json",
        Authorization: "d3d15c2effddf4e11ef9fbcfd066c62878b35039",
        Timezone: "America/Sao_Paulo",
      },
      cache: "no-store",
    },
  );

  if (!res.ok) {
    const errorText = await res.text();
    console.error("Resposta da API:", res.status, errorText);
    throw new Error(`Erro ao buscar dados da API (${res.status})`);
  }

  return res.json();
}

interface PowerPoint {
  time: string;
  total: string;
}

interface PotenciaInsta {
  active: PowerPoint[];
  reactive: PowerPoint[];
  apparent: PowerPoint[];
}

export default function PotenciaInstantanea() {
  const [data, setData] = useState<PotenciaInsta | null>(null);
  const [loading, setLoading] = useState(false);

  const fetchData = async () => {
    setLoading(true);
    try {
      const result = await fetchPowerData();
      setData(result.powerData);
      console.log("RESULTADO API:", result);
      console.log("POWERDATA:", result.powerData);
    } catch (error) {
      console.error("Erro ao buscar dados:", error);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchData();
  }, []);

  const handleExportExcel = () => {
    if (!data) return;

    const rows = data.active.map((_, i) => ({
      time: data.active[i].time,
      active: data.active[i].total,
      reactive: data.reactive[i].total,
      apparent: data.apparent[i].total,
    }));

    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Potência Instantânea");
    XLSX.writeFile(wb, "potencia-instantanea.xlsx");
  };

  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
      {/* Cabeçalho */}
      <div className="flex flex-col gap-4 border-b border-gray-100 p-4 md:flex-row md:items-center md:justify-between dark:border-white/[0.05]">
        <h3 className="font-semibold text-gray-800 dark:text-white">
          Potência Instantânea
        </h3>
        <button
          onClick={handleExportExcel}
          className="rounded-md bg-gray-600 px-4 py-2 text-sm text-white hover:bg-gray-700"
        >
          Exportar Excel
        </button>
      </div>

      {/* Tabela */}
      <div className="max-w-full overflow-x-auto">
        <div className="min-w-[400px]">
          <Table>
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                <TableCell
                  isHeader
                  className="text-theme-xs px-5 py-3 text-center font-medium text-gray-500 dark:text-gray-400"
                >
                  Horário
                </TableCell>
                <TableCell
                  isHeader
                  className="text-theme-xs px-5 py-3 text-center font-medium text-gray-500 dark:text-gray-400"
                >
                  Ativa (kW)
                </TableCell>
                <TableCell
                  isHeader
                  className="text-theme-xs px-5 py-3 text-center font-medium text-gray-500 dark:text-gray-400"
                >
                  Reativa (kVAR)
                </TableCell>
                <TableCell
                  isHeader
                  className="text-theme-xs px-5 py-3 text-center font-medium text-gray-500 dark:text-gray-400"
                >
                  Aparente (kVA)
                </TableCell>
              </TableRow>
            </TableHeader>

            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                <TableRow>
                  <TableCell className="py-6 text-center text-gray-500 dark:text-gray-400">
                    Carregando...
                  </TableCell>
                </TableRow>
              ) : data?.active?.length ? (
                data.active.map((_, i) => (
                  <TableRow key={i}>
                    <TableCell className="px-5 py-4 text-center text-gray-800 dark:text-white/90">
                      {data.active[i]?.time}
                    </TableCell>
                    <TableCell className="px-5 py-4 text-center text-gray-800 dark:text-white/90">
                      {data.active[i]?.total}
                    </TableCell>
                    <TableCell className="px-5 py-4 text-center text-gray-500 dark:text-gray-400">
                      {data.reactive[i]?.total}
                    </TableCell>
                    <TableCell className="px-5 py-4 text-center text-gray-500 dark:text-gray-400">
                      {data.apparent[i]?.total}
                    </TableCell>
                  </TableRow>
                ))
              ) : (
                <TableRow>
                  <TableCell className="py-6 text-center text-gray-500 dark:text-gray-400">
                    Nenhum dado encontrado.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
        </div>
      </div>
    </div>
  );
}
