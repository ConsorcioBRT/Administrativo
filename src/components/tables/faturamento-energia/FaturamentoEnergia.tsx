...
async function getStayDuration(startDate: string, endDate: string) {
  ...
  const res = await fetch(
    `https://gonansen.com.br/endpoints/central-monitoring/revenue-consumption/?start_date=${formatDate(
      startDate,
...
interface ReceitaHeader {
  totalCost: string;
  totalRevenue: string;
  totalProfit: string;
}

export default function FaturamentoEnergia() {
  const [header, setHeader] = useState<ReceitaHeader | null>(null);
  const [data, setData] = useState<ReceitaItem[]>([]);
