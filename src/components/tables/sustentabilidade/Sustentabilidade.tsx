...
async function getEnergyConsumed(startDate: string, endDate: string) {
  ...
  const res = await fetch(
    `https://gonansen.com.br/endpoints/central-monitoring/sustainability-report/?start_date=${formatDate(
      startDate,
...
interface Sustentabilidades {
  co2Avoided: number;
}

export default function Sustentabilidade() {
  const [data, setData] = useState<Sustentabilidades | null>(null);
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Sustentabilidade");
    XLSX.writeFile(wb, "sustentabilidade.xlsx");
  };
